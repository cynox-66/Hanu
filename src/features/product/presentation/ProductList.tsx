...
import { ProductListSkeleton, ProductListEmpty, ProductListError, ProductListNoSearchResults } from './ProductListStates';
...
export const ProductList: React.FC = () => {
  ...
  const [productToArchive, setProductToArchive] = useState<{ id: string; name: string } | null>(null);
      ...
      {searchQuery && filteredProducts.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3" aria-label="Product list">
          {filteredProducts.map((product) => (
            <li key={product.id}>
              <Link to={`/products/${product.id}/edit`} className="block focus:outline-none focus:ring-2 focus:ring-gray-900 rounded-2xl">
                <ProductCard 
                  product={product} 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Are you sure you want to archive <strong>{productToArchive.name}</strong>? It will be removed from the active catalogue but not permanently deleted.
            </p>

...
import { useCustomer, useEditCustomer } from '../hooks';
import { UpdateCustomerDTO } from '../types';

export const EditCustomerPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { customer, isLoading, error: fetchError } = useCustomer(id!);
  const { edit, isSaving } = useEditCustomer();
  const [saveError, setSaveError] = useState<string | null>(null);

  const handleSubmit = async (dto: UpdateCustomerDTO) => {
    if (!customer) return;
    setSaveError(null);
    try {
  ...
  // Guard: archived customers are not editable. Treat them as not found.
  if (fetchError || !customer || customer.status === 'archived') {
    return (
      <div className="flex min-h-screen flex-col bg-gray-50 px-4 py-8 items-center justify-center text-center">
        ...
        <button
          ...
          className="mt-4 font-medium text-gray-900 underline"
        >
          Go back
        </button>
      </div>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <main className="flex-1 px-4 py-6 pb-safe-bottom" id="edit-customer-main">
        {saveError && (
          <div className="mb-6 rounded-xl border border-red-100 bg-red-50 p-4">
            <p className="text-sm font-medium text-red-800">{saveError}</p>
          </div>
        )}
        <CustomerForm initialValues={customer} onSubmit={handleSubmit} isSubmitting={isSaving} />
      </main>
    </div>
  );
};
